...
def convert_json(in_dir, out_dir, yolo_dir):
        ...
        os.chdir("C:/Users/anurag/PycharmProjects/tf_hello/")
        filelst = os.listdir(in_dir + "/out")  # Storing all the names of json files created in the "out" folder inside in_dir in a list named "filelst"
        ...
        for j in filelst:
            with open(os.path.join(in_dir + "/out", j), "r") as infile:  # input the json file directory
                for line in infile:
                    ...
                    header = ["label", "confidence", "top_left_x", "top_left_y", "bottom_right_x",
                              "bottom_right_y"]  # list containing column names
