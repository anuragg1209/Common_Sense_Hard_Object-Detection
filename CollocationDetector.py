...
def collocations_in(obj1, rect1, obj2, rect2):
    ...
    ################################################
    if rect1.part_of(rect2):
        triples.append(construct_triple(obj1=obj1, rel='partOf', obj2=obj2))
    if rect2.part_of(rect1):
        triples.append(construct_triple(obj1=obj2, rel='partOf', obj2=obj1))
    ...
    ################################################
    if rect1.overlaps_with(rect2) or rect2.overlaps_with(rect1):
    ...
    for i in range(num_rows):
        for j in range(i + 1, num_rows):
            ...
            for triple in collocations_in(obj1, rect1, obj2, rect2):
                add_to_counter_map(_map=collocations_map, key=triple)

    update_inverted_index(collocations_map, inverted_index, img_id)
...
def update_inverted_index(collocations_map, inverted_index, img_id):
    ...
    for k, v in collocations_map.items():  # e.g., person overlaps_with ball -> 2
        add_key_to_map_arr(key=k, value=img_id,
                           map_=inverted_index)  # e.g., person overlaps_with ball -> [img_id_5, img_id_72 ...]
